...
import com.mingjie.jf.adapter.CheckEndAdapter;
 ...
 * <p>描   述： 个人中心 - 我的借款 - 已完结fragment
 * <p>创 建 人： shenfei
 * <p>创建时间： 2016-09-27 19:02
 * <p>当前版本： V1.0.0
 * <p>修订历史： (版本、修改时间、修改人、修改内容)
 */
public class BorrowEndFragment extends BaseInvestFragment implements RefreshLayout.OnLoadListener, SwipeRefreshLayout
        .OnRefreshListener, AdapterView.OnItemClickListener
{
    ...
    private List<CheckEndBean.DataBean.ListBean> mDatas = new ArrayList<>();  // 还款中listview的数据
    private CheckEndAdapter adapter;
    ...
    protected View onLoadSuccessView()
    {
        ...
        adapter = new CheckEndAdapter(context, mDatas, mListView);
    ...
    protected LoadingPager.LoadedResult onLoadingData()
    {
        ...
        try
        {
            ...
            CheckEndBean checkEndBean = (CheckEndBean) HttpManager.getInstance().doPost(ServiceName
                    .BORROW_PROUDCTFINISHED, params, CheckEndBean.class);
            if (null == checkEndBean)
            {
                return LoadingPager.LoadedResult.ERROR;
            }
            else if ("000000".equals(checkEndBean.code))
            {
                mDatas = checkEndBean.data.list;
                pageTotal = checkEndBean.data.pageTotal;
    ...
    public void getData()
    {
        ...
        HttpManager.getInstance().doPost(ServiceName.BORROW_PROUDCTFINISHED, params, new HttpCallBack<CheckEndBean>()
        {
            @Override
            public void getHttpCallNull(String str)
            {  // 数据为空
                UIUtils.showToast(getActivity(), R.string.dataError);
                closeRefresh();
            }

            @Override
            public void getHttpCallBack(CheckEndBean list)
            {  // 成功
                closeRefresh();
                if (null != list)
                {
