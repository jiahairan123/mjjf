...
import com.mingjie.jf.adapter.ReturnListAdapter;
 ...
 */
public class ReturnListFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener, RefreshLayout
        .OnLoadListener, View.OnClickListener,AdapterView.OnItemClickListener
{
    ...
    public static final int TYPE_DUE_IN = 1;//待收
    public static final int TYPE_RECEIVED = 2;//已收
    ...
    private List<ReturnListBean> datas = new ArrayList<>();

    private boolean isRefresh = false;//当前是否为刷新
    private boolean canLoad = true;//是否能加载更多

    public static ReturnListFragment getInstance(int type)
    {
    ...
    protected View initView()
    {
        ...
        adapter = new ReturnListAdapter(mActivity, datas);
    ...
    private void getData()
    {
        ...
        HttpManager.getInstance().doPost(ServiceName.RECEIPTPLANCALENDAR_LIST, params, new
                HttpCallBack<ServerResponse<BaseListBean<ReturnListBean>>>()
                {
                    @Override
                    ...
                    @Override
                    public void getHttpCallBack(ServerResponse<BaseListBean<ReturnListBean>> rsp)
                    {
                        closeRefresh();
                        if (rsp == null)
                        {
    ...
    {
        ReturnListBean bean = datas.get(position);
        if(bean!=null)
        {
