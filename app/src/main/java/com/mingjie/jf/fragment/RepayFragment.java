...
import com.mingjie.jf.adapter.RepayAdapter;
 ...
 * <p>描   述： 个人中心 - 我的借款 - 还款中fragment
 * <p>创 建 人： shenfei
 * <p>创建时间： 2016-09-27 14:00
 * <p>当前版本： V1.0.0
 * <p>修订历史： (版本、修改时间、修改人、修改内容)
 */
public class RepayFragment extends BaseInvestFragment implements RefreshLayout.OnLoadListener, SwipeRefreshLayout.OnRefreshListener, AdapterView.OnItemClickListener {
    ...
    private List<RepayBean.DataBean.ListBean> mDatas = new ArrayList<>();  // 还款中listview的数据
    ...
    protected View onLoadSuccessView() {
        ...
        adapter = new RepayAdapter(context, mDatas, mListView);
    ...
    protected LoadingPager.LoadedResult onLoadingData() {
        ...
        try
        {
            ...
            RepayBean repayBean = (RepayBean) HttpManager.getInstance().doPost(ServiceName.BORROW_REPAYMENT, params, RepayBean.class);
            if (null == repayBean)
            {
                return LoadingPager.LoadedResult.ERROR;
            } else if ("000000".equals(repayBean.code))
            {
                mDatas = repayBean.data.list;
                pageTotal = repayBean.data.pageTotal;
    ...
    public void getData()
    {
        ...
        HttpManager.getInstance().doPost(ServiceName.BORROW_REPAYMENT, params, new HttpCallBack<RepayBean>()
        {
            @Override
            ...
            @Override
            public void getHttpCallBack(RepayBean list)
            {  // 成功
                closeRefresh();
                if (null != list)
                {
